...
class CourseController extends Controller
{
    public function getCourses(Request $request)
    {
	    ...
	    return view('frontend._course.index', compact('courses', 'search_term', 'all_categories', 'filters', 'used_tags', 'categories'));
    }
    ...
    public function show(Request $request, $course)
    {
    	...
    	if(Auth::check() && $request->user()->hasEnrolled($course)){
    	  ...
    	  #  return view('frontend._course.enrolled.show', compact('course', 'coupon_code', 'last_watched', 'first_lesson')); 
    	    return view('frontend.categories.course', compact('course', 'coupon_code', 'last_watched', 'first_lesson')); 
    	} else {
    	    
    	   # return view('frontend._course.show', compact('course', 'coupon_code', 'preview_lessons'));
    	    return view('frontend.categories.course', compact('course', 'coupon_code', 'preview_lessons'));
    	}
    }
    ...
    public function getCategory($slug){
        ...
        $courses = Course::where('category_id', '=', $category[0]->id)->get();
        
        return view('frontend.categories.categories', compact('category','courses'));
    }
    
    // public function getSingleCourse(Request $request, Course $course){
    //     $slug = $request->course;
    //     //$slug = $course;
    //     //$course = Course::find($id);
    //     $course = Course::where('slug', $slug)->first();
    //     return view('frontend.categories.course', compact('course'));
    // }
